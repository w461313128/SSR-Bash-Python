...
cd /usr/local
git clone https://github.com/w461313128/shadowsocksr.git
cd ./shadowsocksr
git manyuser
git pull
if [ $1 == "develop" ];then
...
if [[ -d /usr/local/SSR-Bash-Python ]];then
    ...
    cd /usr/local
    git clone https://github.com/w461313128/SSR-Bash-Python.git
    cd AR-B-P-B
    git checkout master
    if [[ $1 == "develop" ]];then
        git checkout develop
...
if [[ $1 == "develop" ]];then
	wget -q -N --no-check-certificate -O /usr/local/bin/ssr https://raw.githubusercontent.com/w461313128/SSR-Bash-Python/master/ssr
