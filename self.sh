...
updateme(){
	...
	wget -q https://raw.githubusercontent.com/w461313128/SSR-Bash-Python/master/version.txt
	...
	if [[ "$version1" == "$version2" ]];then
		...
		if [[ $yn == [Yy] ]];then
			export yn=n
			wget -q -N --no-check-certificate https://raw.githubusercontent.com/w461313128/SSR-Bash-Python/master/install.sh && bash install.sh develop
			sleep 3s
			clear
			ssr || exit 0
		else
...
if [[ $choice == 2 ]];then
	...
	sumdc
	if [[ "$sv" == "$solve" ]];then
		wget -q -N --no-check-certificate https://raw.githubusercontent.com/w461313128/SSR-Bash-Python/master//install.sh && bash install.sh
		sleep 3s
		clear
		ssr || exit 0
	else
...
if [[ $choice == 4 ]];then
	...
	sumdc
	if [[ "$sv" == "$solve" ]];then
		wget -q -N --no-check-certificate https://raw.githubusercontent.com/w461313128/SSR-Bash-Python/master/uninstall.sh &&bash uninstall.sh
		exit 0
	else
		echo "计算错误，正确结果为$solve"
		bash /usr/local/SSR-Bash-Python/self.sh
